...
{
    gpio_put(LCD_PIN_DC, 1);
    spi_write_blocking(LCD_SPI_PORT, ((uint8_t*)&data) + 1, 1);
    spi_write_blocking(LCD_SPI_PORT, ((uint8_t*)&data) + 0, 1);
}
...
void lcd_set_window(uint16_t xs, uint16_t ys, uint16_t xe, uint16_t ye)
{
    ...
    lcd_write_data_16bit(xs + LCD_XSTART);
    lcd_write_data_16bit(xe + LCD_XSTART);

    lcd_write_cmd(LCD_RASET);
    lcd_write_data_16bit(ys + LCD_YSTART);
    lcd_write_data_16bit(ye + LCD_YSTART);
